...
func main() {
	...
	http.HandleFunc("/zip", func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := db.Query("select zip, primaryCity, state, county, timezone, latitude, longitude, irsEstimatedPopulation2015 from zip_code_sample where primaryCity like  '%' || ? || '%'  and type = 'STANDARD'", searchCity)
		if err != nil {
			log.Fatal(err)
		}
		defer rows.Close()
